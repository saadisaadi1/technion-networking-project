...
DcfState::UpdateFailedCw (uint32_t logVal) // *** (Variation of ::UpdateFailedCw for the logVal) ***
{
  ...
  if(m_backoffType == 4){//algorithm 2
	  
		std::random_device rd;  // Seed generator
		std::mt19937 gen(rd()); // Mersenne Twister engine

		// Define a distribution for double values between 0 and 0.2
		std::uniform_real_distribution<double> distribution(0.0, 1.0);

		// Generate random double values
		double randomValue = distribution(gen);
		if(randomValue < 0.2){
			m_cw = std::min (m_cw/2 + 1, m_cwMax);
		}else{
			if(m_cw > m_cwMax/8){
				m_cw = std::min ( (uint32_t)(2 * m_cw + 1), m_cwMax);
			}
			else {
				m_cw = std::min ( (uint32_t)((randomValue - 0.2 + 2) * m_cw + 1), m_cwMax);
			}
		}
	  
  }
  
  if(m_backoffType == 5){// algorithm 3
